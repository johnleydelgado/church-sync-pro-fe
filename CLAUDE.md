...
- **URL-encode an email anywhere it goes into a query string or a link** (`encodeURIComponent`). A `+` in an address reaches Express as a space, the user is "not found", and the symptom is an unrelated-looking "Oops! Something went wrong" at login or an "invalid invitation". Found on staging 2026-09-25; the older `?email=${email}` builders and the backend's invite/reset links were all wrong.
